/**
 * @Description
 * @author Fighter
 * @create 2022-03-19 11:11
 */


package day27;

import java.util.ArrayList;
import java.util.Arrays;
import java.util.List;

/**
 * 986. 区间列表的交集
 * 给定两个由一些 闭区间 组成的列表，firstList 和 secondList ，其中 firstList[i] = [starti, endi] 而 secondList[j] = [startj, endj] 。每个区间列表都是成对 不相交 的，并且 已经排序 。
 *
 * 返回这 两个区间列表的交集 。
 *
 * 形式上，闭区间 [a, b]（其中 a <= b）表示实数 x 的集合，而 a <= x <= b 。
 *
 * 两个闭区间的 交集 是一组实数，要么为空集，要么为闭区间。例如，[1, 3] 和 [2, 4] 的交集为 [2, 3] 。
 *
 *
 *
 * 示例 1：
 *
 *
 * 输入：firstList = [[0,2],[5,10],[13,23],[24,25]], secondList = [[1,5],[8,12],[15,24],[25,26]]
 * 输出：[[1,2],[5,5],[8,10],[15,23],[24,24],[25,25]]
 * 示例 2：
 *
 * 输入：firstList = [[1,3],[5,9]], secondList = []
 * 输出：[]
 * 示例 3：
 *
 * 输入：firstList = [], secondList = [[4,8],[10,12]]
 * 输出：[]
 * 示例 4：
 *
 * 输入：firstList = [[1,7]], secondList = [[3,10]]
 * 输出：[[3,7]]
 *
 *
 * 提示：
 *
 * 0 <= firstList.length, secondList.length <= 1000
 * firstList.length + secondList.length >= 1
 * 0 <= starti < endi <= 109
 * endi < starti+1
 * 0 <= startj < endj <= 109
 * endj < startj+1
 */

public class IntervalIntersection {
    /**
     * 减少相应步骤
     * @param firstList
     * @param secondList
     * @return
     */
    public int[][] intervalIntersection(int[][] firstList, int[][] secondList) {
        if (firstList.length==0 || secondList.length==0 ){
            return new int[][]{};
        }
        List<int[]> resList = new ArrayList<>();
        int f=0,s=0;
        while (f<firstList.length && s< secondList.length){
            int lv = Math.max(firstList[f][0], secondList[s][0]);
            int rv = Math.min(firstList[f][1], secondList[s][1]);
            if (lv<=rv){
                resList.add(new int[]{lv, rv});
            }

            if (firstList[f][1]>secondList[s][1]){
                s++;
            }else {
                f++;
            }
        }

        return resList.toArray(new int[resList.size()][]);
    }

    /**
     * 暴力法
     * @param firstList
     * @param secondList
     * @return
     */
    public int[][] intervalIntersection1(int[][] firstList, int[][] secondList) {

        if (firstList.length==0 || secondList.length==0 ){
            return new int[][]{};
        }
        List<int[]> resList = new ArrayList<>();
        int F = firstList.length;
        int S = secondList.length;
        int f=0,s=0;
        while (f<F && s<S){
            if (firstList[f][0] > secondList[s][1]){
                s++;
                continue;
            }

            if (secondList[s][0] > firstList[f][1]){
                f++;
                continue;
            }

            if (firstList[f][0]>secondList[s][0]){
                if (firstList[f][1]<secondList[s][1]){
                    resList.add(new int[]{firstList[f][0], firstList[f][1]});
                    f++;
                }else {
                    resList.add(new int[]{firstList[f][0], secondList[s][1]});
                    s++;
                }
            }else {
                if (firstList[f][1]<secondList[s][1]){
                    resList.add(new int[]{secondList[s][0], firstList[f][1]});
                    f++;
                }else {
                    resList.add(new int[]{secondList[s][0], secondList[s][1]});
                    s++;
                }
            }
        }

        int[][] res = new int[resList.size()][2];
        for (int i=0;i<resList.size();i++){
            res[i] = resList.get(i);
        }

        return res;
    }


    public static void main(String[] args) {
        int[][] firstList = {{0,2},{5,10},{13,23},{24,25}}, secondList = {{1,5},{8,12},{15,24},{25,26}};

        IntervalIntersection intersection = new IntervalIntersection();
        int[][] ints = intersection.intervalIntersection(firstList, secondList);
        for (int[] ints1:ints){
            System.out.println(Arrays.toString(ints1));
        }
    }
}
